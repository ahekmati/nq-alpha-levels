...
def parse_args():
    ...
    p.add_argument("--symbol", default="MNQ", help="Preferred research symbol")
    p.add_argument("--start", default="2020-01-01")
    p.add_argument("--end", default=None)
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=18812)
    p.add_argument("--bars-ahead", type=int, default=12)
    p.add_argument("--long-threshold", type=float, default=0.006)
    p.add_argument("--short-threshold", type=float, default=0.006)
    return p.parse_args()
...
def merge_features(h1: pd.DataFrame, d1_feat: pd.DataFrame, cfg: HorizonConfig) -> pd.DataFrame:
    ...
    daily_cols = [
        ...
        if c not in ["open", "high", "low", "close", "tick_volume", "spread", "real_volume"]
    ]
    ...
    for name, model in model_dict.items():
        ...
        for fold, (tr_idx, te_idx) in enumerate(splitter.split(X)):
            ...
            rows.append({
                ...
                "threshold": np.nan,
                "signals": np.nan,
                "precision_at_thr": np.nan,
                "mean_fwd_ret_at_thr": np.nan,
                "median_fwd_ret_at_thr": np.nan,
                "directional_hitrate_at_thr": np.nan,
            })
        ...
        for thr in thresholds:
            ...
            if take.sum() == 0:
                precision_thr = np.nan
                avg_fwd = np.nan
                median_fwd = np.nan
                hitrate = np.nan
            else:
                precision_thr = float(np.mean(y[take]))
            ...
            rows.append({
                ...
                "precision_at_thr": precision_thr,
...
def main():
    ...
    print("\n" + "=" * 80)
    print("LONG MODELS @ 0.70 threshold")
    print("=" * 80)
    long_thr = long_eval[
        (long_eval["fold"] == "all") & (long_eval["threshold"] == 0.70)
    ...
    print("\n" + "=" * 80)
    print("SHORT MODELS @ 0.70 threshold")
    print("=" * 80)
    short_thr = short_eval[
        (short_eval["fold"] == "all") & (short_eval["threshold"] == 0.70)
    ].sort_values("roc_auc", ascending=False)
    print(short_thr[
        [
            "model",
    ...
    print("\n" + "=" * 80)
    print("TOP FEATURES FOR LONG TRADES")
    print("=" * 80)
    print(long_imp.head(15).to_string(index=False))

    print("\n" + "=" * 80)
    print("TOP FEATURES FOR SHORT TRADES")
    print("=" * 80)
