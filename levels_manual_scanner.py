...
def print_header(cfg: ScannerConfig):
    print("=" * 92)
    print(f"LEVELS MANUAL SCANNER | {cfg.symbol} {cfg.tf_label} | threshold={cfg.threshold:.0%} | SL={cfg.sl_atr:.2f} ATR | TP={cfg.tp_r:.2f}R")
    print(f"session={cfg.session} | max_dist={cfg.max_dist_atr:.2f} ATR | top={cfg.top} | mode={'ONCE' if cfg.once else 'WATCH'}")
...
def print_candidates(candidates: list[dict], current_price: float, atr: float):
    print(f"\n[{utc_now().strftime('%Y-%m-%d %H:%M:%S UTC')}] price={current_price:.2f} atr={atr:.2f} candidates={len(candidates)}")
    if not candidates:
        print("No manual limit candidates right now.")
        return
    print(f"{'Level':>10} {'Entry':>10} {'Stop':>10} {'Target':>10} {'DistATR':>8} {'Score':>8} {'Touch':>6} {'Age':>6}  Signal")
    print("-" * 92)
    for c in candidates:
        print(
            f"{c['level']:>10.2f} {c['entry']:>10.2f} {c['stop']:>10.2f} {c['target']:>10.2f} "
            f"{c['distance_atr']:>8.2f} {c['score']:>8.1%} {c['touches']:>6} {c['age_bars']:>6}  {c['signal']}"
        )
...
def main():
    ...
    parser.add_argument("--max-dist-atr", type=float, default=1.50)
