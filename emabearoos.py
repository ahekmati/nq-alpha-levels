...
BACKTEST_TRADES_CSV = "ema100_bear_backtest_extended_trades.csv"
BACKTEST_SUMMARY_CSV = "ema100_bear_backtest_extended_summary.csv"
WF_SUMMARY_CSV = "ema100_bear_backtest_extended_walkforward_summary.csv"
...
def walk_forward_rf_expanding(features_df, feature_cols):
    ...
    first_test_start = split_start + pd.DateOffset(months=INITIAL_TRAIN_MONTHS)
...
def run_capital_backtest(trades_df, label):
    if trades_df.empty:
...
pred_df, wf_df = walk_forward_rf_expanding(features_df, feature_cols)

if pred_df.empty:
    print("No OOS predictions generated. Cannot run extended backtest.")
else:
    ...
    postsample = filtered[filtered["entry_time"] >= pd.Timestamp("2024-01-01")].copy()

    full_summary, full_bt = run_capital_backtest(filtered, "full_filtered_oos")
    post_summary, post_bt = run_capital_backtest(postsample, "postsample_2024_2026")

    wf_df.to_csv(WF_SUMMARY_CSV, index=False)
    filtered.to_csv(BACKTEST_TRADES_CSV, index=False)

    summary_rows = []
    if full_summary is not None:
        summary_rows.append(full_summary)
    if post_summary is not None:
        summary_rows.append(post_summary)
    pd.DataFrame(summary_rows).to_csv(BACKTEST_SUMMARY_CSV, index=False)

    print("\n" + "=" * 90)
    print("EXTENDED WALK-FORWARD BACKTEST")
    print("=" * 90)
    print(f"Symbol: {symbol}")
    print(f"Strategy: RF >= {RF_THRESHOLD:.2f} | 1 MNQ | 1.0 ATR Stop / 1.5 ATR Target")
    print(f"Feature rows built: {len(features_df)}")
    print(f"Total OOS scored trades: {len(pred_df)}")
    print(f"Filtered OOS trades: {len(filtered)}")
    print(f"Walk-forward folds: {len(wf_df)}")

    if not wf_df.empty:
        print("\n--- Walk-Forward Diagnostics ---")
        print(wf_df.to_string(index=False))
        print(f"\nMean AUC: {wf_df['auc'].mean():.4f}")
        print(f"Mean Accuracy: {wf_df['accuracy'].mean():.4f}")

    if full_summary is not None:
        print("\n" + "-" * 90)
        print("FULL FILTERED OOS RESULTS")
        print("-" * 90)
        print(f"Period:               {full_summary['start_entry']} -> {full_summary['end_exit']}")
        print(f"Initial Capital:      ${full_summary['initial_capital_usd']:,.2f}")
        print(f"Final Capital:        ${full_summary['final_capital_usd']:,.2f}")
        print(f"Total P&L:            ${full_summary['total_pnl_usd']:,.2f}")
        print(f"Total Return:         {full_summary['total_return_pct']:.2f}%")
        print(f"Total Trades:         {full_summary['total_trades']}")
        print(f"Win Rate:             {full_summary['win_rate_pct']:.2f}%")
        print(f"Profit Factor:        {full_summary['profit_factor']:.2f}")
        print(f"Sharpe Ratio:         {full_summary['sharpe']:.2f}")
        print(f"Max Drawdown:         ${full_summary['max_drawdown_usd']:,.2f} ({full_summary['max_drawdown_pct']:.2f}%)")
        print(f"Avg Winner:           ${full_summary['avg_winner_usd']:,.2f}")
        print(f"Avg Loser:            ${full_summary['avg_loser_usd']:,.2f}")
        print(f"Avg P&L per Trade:    ${full_summary['avg_pnl_per_trade_usd']:,.2f}")
        print(f"Avg Bars Held:        {full_summary['avg_bars_held']:.2f}")

    if post_summary is not None:
        print("\n" + "-" * 90)
        print("POST-SAMPLE 2024-2026 RESULTS")
        print("-" * 90)
        print(f"Period:               {post_summary['start_entry']} -> {post_summary['end_exit']}")
        print(f"Initial Capital:      ${post_summary['initial_capital_usd']:,.2f}")
        print(f"Final Capital:        ${post_summary['final_capital_usd']:,.2f}")
        print(f"Total P&L:            ${post_summary['total_pnl_usd']:,.2f}")
        print(f"Total Return:         {post_summary['total_return_pct']:.2f}%")
        print(f"Total Trades:         {post_summary['total_trades']}")
        print(f"Win Rate:             {post_summary['win_rate_pct']:.2f}%")
        print(f"Profit Factor:        {post_summary['profit_factor']:.2f}")
        print(f"Sharpe Ratio:         {post_summary['sharpe']:.2f}")
        print(f"Max Drawdown:         ${post_summary['max_drawdown_usd']:,.2f} ({post_summary['max_drawdown_pct']:.2f}%)")
        print(f"Avg Winner:           ${post_summary['avg_winner_usd']:,.2f}")
        print(f"Avg Loser:            ${post_summary['avg_loser_usd']:,.2f}")
        print(f"Avg P&L per Trade:    ${post_summary['avg_pnl_per_trade_usd']:,.2f}")
        print(f"Avg Bars Held:        {post_summary['avg_bars_held']:.2f}")
    else:
        print("\n" + "-" * 90)
        print("POST-SAMPLE 2024-2026 RESULTS")
        print("-" * 90)
        print("No filtered trades were generated in 2024-2026 with the current settings.")

    if not filtered.empty:
        print("\n" + "-" * 90)
        print("LAST 10 FILTERED TRADES (Most Recent First)")
        print("-" * 90)
        last10 = filtered.sort_values("entry_time").tail(10).iloc[::-1]
        for i, row in last10.iterrows():
            ...
            print(f"  RF Prob: {row['rf_prob']:.3f}")

    print("\nSaved extended trades to:", BACKTEST_TRADES_CSV)
    print("Saved extended summary to:", BACKTEST_SUMMARY_CSV)
