...
# ATR-based stop settings
ATR_PERIOD = 14
STOP_ATR_MULT = 1.0          # 1.0x ATR as base
MIN_STOP_POINTS = 60.0       # clamp lower bound
MAX_STOP_POINTS = 200.0      # clamp upper bound

# RSI settings (asymmetric, based on your short edge)
RSI_PERIOD = 7
RSI_LONG_THRESHOLD = 50      # require more strength for longs
RSI_SHORT_THRESHOLD = 55     # allow shorts more readily

# ---------- FEATURE TOGGLES (0=off, 1=on) ---------- #
USE_REGIME_PERSISTENCE = 1   # require regime to persist N bars
REGIME_PERSIST_BARS = 2

USE_MIN_HOLD = 1             # require trade held at least N bars before regime-flip exit
MIN_HOLD_BARS = 2

USE_ENTRY_COOLDOWN = 1       # wait X bars after close before new entry
...
def backtest_long_short_with_atr_stop(data: pd.DataFrame, regime: pd.Series) -> pd.DataFrame:
    d = data.copy()
    d["regime"] = regime
    ...
    for idx, (t, row) in enumerate(d.iterrows()):
        ...
        atr = float(atr_series.loc[t])

        # decrement cooldown
        if cooldown_remaining > 0:
            cooldown_remaining -= 1

        # manage existing position: ATR-based stop
        if position is not None:
            if position["side"] == "long":
                ...
                if low_price <= stop_price:
                    ...
                    position = None
                    if USE_ENTRY_COOLDOWN:
                        cooldown_remaining = ENTRY_COOLDOWN_BARS
        ...
        # regime flip logic + RSI gate for new entries / exits
        if regime_shift.loc[t]:
            # handle exits on flip with min-hold
            if position is not None:
                # compute bar hold length using stored entry index if available
                held_ok = True
                if USE_MIN_HOLD:
                    held_ok = (idx - position["entry_idx"]) >= MIN_HOLD_BARS

                if held_ok:
                    # exit at open on flip
                    trades.append(close_trade(position, t, open_price, "regime_flip"))
                    position = None
                    if USE_ENTRY_COOLDOWN:
                        cooldown_remaining = ENTRY_COOLDOWN_BARS

            # no pyramiding: only consider new entry if flat
            if position is None and cooldown_remaining == 0:
                # calculate ATR-based stop distance
                stop_points = max(MIN_STOP_POINTS, min(MAX_STOP_POINTS, atr * STOP_ATR_MULT))
                ...
                if reg == "bull":
                    if rsi > RSI_LONG_THRESHOLD:
                        position = {
                            ...
                            "stop_price": open_price - stop_points,
                            "entry_idx": idx,
                        }
...
def print_trades(trades_df: pd.DataFrame, mode: str):
    ...
    print(f"ATR period: {ATR_PERIOD}, STOP_ATR_MULT: {STOP_ATR_MULT}, clamp=[{MIN_STOP_POINTS},{MAX_STOP_POINTS}]")
    print(f"Use regime persistence: {USE_REGIME_PERSISTENCE} ({REGIME_PERSIST_BARS} bars)")
    print(f"Use min hold: {USE_MIN_HOLD} ({MIN_HOLD_BARS} bars)")
    print(f"Use entry cooldown: {USE_ENTRY_COOLDOWN} ({ENTRY_COOLDOWN_BARS} bars)\n")
...
def main():
    ...
    data = bars.join(features[["log_ret", "rv_10", "rsi", "atr"]], how="left")
    data = data.join(regime_series.rename("regime"), how="left")

    trades_df = backtest_long_short_with_atr_stop(data, data["regime"])
