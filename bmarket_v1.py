...
STRICT_NO_FLIP = True          # TODO: enforce — currently declared but not yet applied in decision logic
STRICT_ONE_POSITION_ONLY = True  # TODO: enforce — currently declared but not yet applied in decision logic
...
# H1 bar duration in hours — used to accurately determine bar close time
H1_BAR_DURATION_HOURS = 1
...
def get_latest_closed_bar(df: pd.DataFrame) -> pd.Series:
    """
    FIX #5: A bar whose open time is T is only fully closed at T + H1_BAR_DURATION_HOURS.
    We filter on bar_close_time <= now_utc so we never act on a bar that is still forming.
    Previously the filter was `bar_open_time <= now_utc - 1h`, which is equivalent but
    was semantically ambiguous. The explicit bar-close calculation makes the intent clear.
    """
    now_utc = datetime.now(timezone.utc)
    bar_close_times = df["time"] + pd.Timedelta(hours=H1_BAR_DURATION_HOURS)
...
def candidate_filling_modes(symbol: str) -> List[int]:
    """
    FIX #2: The original code used `object()` as a sentinel for missing MT5 attributes,
    which creates a new unique object each call. That means the dict lookup
    `symbol_mode in symbol_to_order` could never match the sentinel key, silently
    bypassing the preferred filling mode and always falling through to generic candidates.

    Fix: use None as the sentinel. We also guard against None keys in the dict to avoid
    a None-keyed entry accidentally matching a None symbol_mode on a misconfigured symbol.
    """
    info = mt5.symbol_info(symbol)
    if info is None:
        return []

    # Resolve MT5 constants with None as a safe sentinel (not object())
    ...
    # Only build the mapping for constants that actually resolved
    ...
    # Preferred: the filling mode the symbol advertises
    if symbol_mode is not None and symbol_mode in symbol_to_order:
        preferred = symbol_to_order[symbol_mode]
        if preferred is not None:
            candidates.append(preferred)

    # Fallback: try all common order filling modes in priority order
...
def build_short_request(symbol: str, lot: float, stop_points: float, tp_points: float, signal_time: str) -> Dict[str, Any]:
    ...
    # FIX #1: Compute raw TP distance and detect if it rounded down to zero.
    # If tp_points > 0 was requested but the distance rounded to 0.0, log a warning
    # and treat it as no-TP rather than silently sending tp=0.0 as if TP was never set.
    tp_distance = 0.0
    if tp_points > 0:
...
def order_send_checked(base_request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # FIX #3: Guard against an empty candidate list before indexing with [0].
    # Previously this would raise an unguarded IndexError if candidate_filling_modes()
    # returned [] (e.g. symbol_info unavailable or no recognised filling constants).
    if chosen_fill is None:
        ...
        logger.warning(
            f"No filling mode passed order_check; falling back to first candidate: {chosen_fill}"
        )
