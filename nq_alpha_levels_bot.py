"""
levels_alert.py — Key Level Scanner + Telegram Alert
Run on cron every 30 minutes during trading hours.
...
Cron example (every 30min, Mon-Fri, 08:00-21:00 UTC):
  */30 8-21 * * 1-5 cd /home/ronin/projects/mt5-python && \
    .venv/bin/python levels_alert.py >> logs/alert_cron.log 2>&1
...
MAX_DIST_ATR  = 5.0     # only alert levels within this many ATR of price
                         # (set higher to see further levels too)
...
# ─────────────────────────────────────────────────────────────────────────────
# MT5 + INDICATORS (reuse same logic as levels_ml.py)
# ─────────────────────────────────────────────────────────────────────────────
