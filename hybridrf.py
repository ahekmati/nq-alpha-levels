...
RSI_PERIOD = 10
RSI_ENTRY_MAX = 55.0        # only short if RSI is still weak
RSI_FORCE_EXIT_MIN = 35.0   # exit short if RSI gets too washed out

USE_TRAILING_STOP = True
TRAILING_STOP_POINTS = 80

ALLOW_NEW_SHORTS_ONLY_IN_BEAR = True
# ---------------------------------------- #
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    f["ema_fast"] = f["close"].ewm(span=10, adjust=False).mean()
    f["ema_slow"] = f["close"].ewm(span=30, adjust=False).mean()
    f["trend_down"] = f["ema_fast"] < f["ema_slow"]

    return f.dropna(subset=["log_ret", "rv_10", "rsi", "ema_fast", "ema_slow"])
...
def fit_hmm(features: pd.DataFrame) -> pd.Series:
    ...
    features["regime"] = np.where(features["state"] == bear_state, "bear", "bull")
    features["bear_state_mean_log_ret"] = state_means.loc[bear_state]
    features["bull_state_mean_log_ret"] = state_means.loc[bull_state]
...
def backtest_bear_only_short(data: pd.DataFrame, regime: pd.Series) -> pd.DataFrame:
    d = data.copy()
    d["regime"] = regime
    d["regime"] = d["regime"].ffill()
    d = d.dropna(subset=["regime", "rsi", "trend_down"])

    signals = d["regime"]
    rsi_series = d["rsi"]
    trend_down = d["trend_down"]
    ...
    for t, row in d.iterrows():
        ...
        reg = signals.loc[t]
        rsi = float(rsi_series.loc[t])
        downtrend = bool(trend_down.loc[t])
        ...
        if position is not None:
            ...
            if high_price >= stop_price:
                ...
                # optional trailing stop for open profits
                if USE_TRAILING_STOP:
                    new_stop = min(position["stop_price"], close_price + TRAILING_STOP_POINTS)
                    position["stop_price"] = float(new_stop)

                # exit if regime leaves bear mode
                if position is not None and reg != "bear":
                    trades.append(close_trade(position, t, open_price, "regime_flip"))
                    position = None

                # exit if RSI becomes too oversold / risk of snapback
                elif position is not None and rsi <= RSI_FORCE_EXIT_MIN:
                    trades.append(close_trade(position, t, open_price, "rsi_exit"))
                    position = None

        # ----- open new short only in bear regime -----
        if position is None:
            can_short = True

            if ALLOW_NEW_SHORTS_ONLY_IN_BEAR and reg != "bear":
                can_short = False

            if can_short and reg == "bear" and downtrend and rsi < RSI_ENTRY_MAX:
                position = {
                    "side": "short",
                    "entry_time": t,
        ...
        # if bear regime just started, allow fresh short on shift if conditions fit
        if position is None and regime_shift.loc[t]:
            if reg == "bear" and downtrend and rsi < RSI_ENTRY_MAX:
                position = {
                    "side": "short",
                    "entry_time": t,
                    "entry_price": open_price,
...
def compute_metrics(trades_df: pd.DataFrame, mode: str) -> dict:
    ...
    rsi_exits = (trades_df["exit_reason"] == "rsi_exit").sum()
...
def print_trades(trades_df: pd.DataFrame, mode: str):
    ...
    print(f"\n--- {tf_label} Bear-Market Short Trades for {SYMBOL} ---")
...
def print_metrics(metrics: dict):
    ...
    print(f"\n{g}--- Bear Market Performance Metrics ---{e}")
    ...
    print(f"{g}RSI exits: {metrics['rsi_exits']}{e}")
...
def main():
    ...
    data = bars.join(features[["log_ret", "rv_10", "rsi", "ema_fast", "ema_slow", "trend_down"]], how="left")
    data = data.join(regime_series.rename("regime"), how="left")

    trades_df = backtest_bear_only_short(data, data["regime"])
