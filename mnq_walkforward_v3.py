...
OUTDIR = Path("validation_run_441_v3")
...
def walk_forward_validate(df: pd.DataFrame, p: StrategyParams, train_years: int = 3, test_months: int = 12) -> pd.DataFrame:
...
def neighbor_params() -> list[StrategyParams]:
    ...
    variants = [
        base,
        StrategyParams(tp_atr_mult=2.8),
        ...
        StrategyParams(trail_atr_mult_after_tp=0.9),
        StrategyParams(trail_atr_mult_after_tp=1.3),
        StrategyParams(commission_per_side_usd=0.62, slippage_points_per_side=0.5),
...
def main():
    ...
    base = StrategyParams()
    dctx = build_daily_context(d1, base)
    hfeat = build_hourly_features(h1, base)
    df = merge_context(hfeat, dctx)
    df = build_signals(df, base)
    ...
    for idx, p in enumerate(neighbor_params(), start=1):
        ...
        wf = walk_forward_validate(df, p, train_years=3, test_months=12)
        if not wf.empty:
            wf["variant_id"] = idx
            all_wf.append(wf)
    ...
    merged = merged.sort_values(["profit_factor", "total_points"], ascending=[False, False])
    ...
    report = {
        ...
        "notes": [
            ...
            "Optional costs/slippage included in neighbor variants",
    ...
    print("Top variants:")
    cols = [c for c in ["variant_id", "profit_factor", "total_points", "n_trades", "win_rate", "max_drawdown_points", "pct_trail_stop", "wf_avg_pf", "wf_total_points"] if c in merged.columns]
