"""
HMM Regime Strategy v4 — @MNQ H1
Live-ready system incorporating every improvement from development conversation.
...
VOL_PCT_NO_ENTRY  = 70.0          # vol percentile above which no new entries
VOL_PCT_ONE_LOT   = 50.0          # vol percentile above which max 1 lot (no scale)
...
RSI_LONG_BASE          = 45
RSI_SHORT_BASE         = 55
RSI_TIGHTEN_STEP       = 3        # tighten per LOSS_STREAK_TIER_SIZE losses
...
STOP_HORIZON_BARS      = 6        # expected hold in H1 bars
...
BASE_CONTRACTS         = 1
MAX_CONTRACTS          = 2
PERSIST_FOR_SCALE      = 10       # regime must persist >= this to allow 2 lots
SCALE_QUALITY_THRESH   = 65       # quality score must be >= this for 2 lots
...
# ── Entry filters ─────────────────────────────────────────────────────────────
CONFIRM_BARS           = 1        # bars after regime shift before entry allowed
MIN_HOLD_BARS          = 3        # min bars since last exit before new entry

# ── Session filter ─────────────────────────────────────────────────────────────
ENABLE_SESSION_FILTER  = True
SESSION_WINDOWS        = [(7, 10), (13, 20)]   # UTC hours (London open + NY)

# ── Drawdown brake ────────────────────────────────────────────────────────────
DD_BRAKE_PCT           = 0.12     # pause entries if equity DD > 12% from peak
DD_BRAKE_RECOVER       = 0.50     # resume when 50% of drawdown recovered
...
def backtest(data: pd.DataFrame,
    ...
    for idx, (t, row) in enumerate(rows):
        ...
        if can_enter:
            # Kalman confirmer: velocity must agree with regime direction
            kalman_confirms = (
                (pending_regime == "bull" and kvel > 0) or
                (pending_regime == "bear" and kvel < 0)
            )

            if not kalman_confirms:
                pass  # wait for Kalman to agree
...
def print_trades(df: pd.DataFrame, label: str = "") -> None:
    ...
    W = 96
    print(f"\n{'═'*W}")
    print(f"  TRADES — {label}  |  {SYMBOL}  {TIMEFRAME}")
...
class LiveTrader:
    ...
    def on_bar_close(self, bar: pd.Series,
        ...
        if self.position is None and regime != "uncertain":
            ...
            # Simplified quality proxy for live (no full rolling history):
            # just check vol percentile as main gate
